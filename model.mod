...
%--------------------------------------------------------------------------------
% Matlab Output: CEV and Grouped Plots
%--------------------------------------------------------------------------------
...
idx_V_H = strmatch('V_H', M_.endo_names, 'exact'); V_H_init = oo_.endo_simul(idx_V_H, 1); 
idx_V_M = strmatch('V_M', M_.endo_names, 'exact'); V_M_init = oo_.endo_simul(idx_V_M, 1); 
idx_V_S = strmatch('V_S', M_.endo_names, 'exact'); V_S_init = oo_.endo_simul(idx_V_S, 1); 
idx_C_H = strmatch('C_H', M_.endo_names, 'exact'); C_H_init = oo_.endo_simul(idx_C_H, 1);
idx_C_M = strmatch('C_M', M_.endo_names, 'exact'); C_M_init = oo_.endo_simul(idx_C_M, 1);
idx_C_S = strmatch('C_S', M_.endo_names, 'exact'); C_S_init = oo_.endo_simul(idx_C_S, 1);

% Column 2 is the shock, periods after are the transition
V_H_trans = oo_.endo_simul(idx_V_H, 2:options_.periods+1); 
V_M_trans = oo_.endo_simul(idx_V_M, 2:options_.periods+1); 
V_S_trans = oo_.endo_simul(idx_V_S, 2:options_.periods+1); 

% CEV Formula 
CEV_H = (1 + (V_H_trans - V_H_init).*(1-betta_H).*(1-sigma) ./ (C_H_init^(1-sigma))).^(1/(1-sigma)) - 1;
CEV_M = (1 + (V_M_trans - V_M_init).*(1-betta_M).*(1-sigma) ./ (C_M_init^(1-sigma))).^(1/(1-sigma)) - 1;
CEV_S = (1 + (V_S_trans - V_S_init).*(1-betta_S).*(1-sigma) ./ (C_S_init^(1-sigma))).^(1/(1-sigma)) - 1;
...
plot(time_axis, CEV_H * 100, 'b', time_axis, CEV_M * 100, 'r', time_axis, CEV_S * 100, 'g', 'LineWidth', 2);
...
get_dev = @(var) (oo_.endo_simul(strmatch(var, M_.endo_names, 'exact'), 2:options_.periods+1) - ...
                  oo_.endo_simul(strmatch(var, M_.endo_names, 'exact'), 1)) ...
                  ./ oo_.endo_simul(strmatch(var, M_.endo_names, 'exact'), 1) * 100;
              
% Annualized Percentage Point deviation:
get_pp  = @(var) (oo_.endo_simul(strmatch(var, M_.endo_names, 'exact'), 2:options_.periods+1) - ...
                  oo_.endo_simul(strmatch(var, M_.endo_names, 'exact'), 1)) * 400; 

% Pure Levels:
get_lvl = @(var) oo_.endo_simul(strmatch(var, M_.endo_names, 'exact'), 2:options_.periods+1);
get_exo_lvl = @(var) oo_.exo_simul(2:options_.periods+1, strmatch(var, M_.exo_names, 'exact'))';
end;


% ------------- FIGURE 1: Macro & Household Dynamics -------------

fig1 = figure('Name', 'Macro & Household Dynamics', 'Position', [100, 100, 1200, 800]);

% Subplot 1: Consumption (% dev)
subplot(3,3,1);
plot(time_axis, get_dev('C_H'), 'b', time_axis, get_dev('C_M'), 'r', time_axis, get_dev('C_S'), 'g', 'LineWidth', 2);
title('Consumption (% Dev)'); legend('C_H', 'C_M', 'C_S', 'Location', 'best'); grid on; xlim([1 options_.periods]);

% Subplot 2: Labour (% dev)
subplot(3,3,2);
plot(time_axis, get_dev('L_H'), 'b', time_axis, get_dev('L_M'), 'r', 'LineWidth', 2);
title('Labour (% Dev)'); legend('L_H', 'L_M', 'Location', 'best'); grid on; xlim([1 options_.periods]);

% Subplot 3: Income (% dev)
subplot(3,3,3);
plot(time_axis, get_dev('INC_H'), 'b', time_axis, get_dev('INC_M'), 'r', time_axis, get_dev('INC_S'), 'g', 'LineWidth', 2);
title('Incomes (% Dev)'); legend('INC_H', 'INC_M', 'INC_S', 'Location', 'best'); grid on; xlim([1 options_.periods]);

% Subplot 4: Net Wealth (% dev)
subplot(3,3,4);
plot(time_axis, get_dev('NW_H'), 'b', time_axis, get_dev('NW_M'), 'r', time_axis, get_dev('NW_S'), 'g', 'LineWidth', 2);
title('Net Wealth (% Dev)'); legend('NW_H', 'NW_M', 'NW_S', 'Location', 'best'); grid on; xlim([1 options_.periods]);

% Subplot 5: Output, labour, capital (% dev)
subplot(3,3,5);
plot(time_axis, get_dev('Y'), 'b', time_axis, get_dev('L'), 'r', time_axis, get_dev('K'), 'g', 'LineWidth', 2);
title('Aggregates (% Dev)'); legend('Y', 'L', 'K', 'Location', 'best'); grid on; xlim([1 options_.periods]);

% Subplot 6: Firms (% dev)
subplot(3,3,6);
plot(time_axis, get_dev('mc'), 'b', time_axis, get_dev('I'), 'r', time_axis, get_dev('w'), 'g', 'LineWidth', 2);
title('Firm Metrics (% Dev)'); legend('mc', 'I', 'w', 'Location', 'best'); grid on; xlim([1 options_.periods]);

% Subplot 7: Prices (% dev)
subplot(3,3,7);
plot(time_axis, get_dev('s'), 'b', time_axis, get_dev('q'), 'r', time_axis, get_dev('r'), 'g', 'LineWidth', 2);
title('Prices (% Dev)'); legend('s', 'q', 'r', 'Location', 'best'); grid on; xlim([1 options_.periods]);
...
fig2 = figure('Name', 'Financial, Housing & Public Sector', 'Position', [150, 150, 1200, 800]);

% Subplot 1: Deposits (% dev)
subplot(3,3,1);
plot(time_axis, get_dev('m_H'), 'b', time_axis, get_dev('m_cH'), 'r', 'LineWidth', 2);
title('H Deposits (% Dev)'); legend('Bank Deposits (m_H)', 'RFA Deposits (m_cH)', 'Location', 'best'); grid on; xlim([1 options_.periods]);

% Subplot 2: Debt & Bonds (% dev)
subplot(3,3,2);
plot(time_axis, get_dev('n_M'), 'r', time_axis, get_dev('b_S'), 'g', 'LineWidth', 2);
title('Debt & Bonds (% Dev)'); legend('M Mortgages (n_M)', 'S Bonds (b_S)', 'Location', 'best'); grid on; xlim([1 options_.periods]);

% Subplot 3: Housing & Constraints (% dev)
subplot(3,3,3);
plot(time_axis, get_dev('H_M'), 'b', time_axis, get_dev('H_S'), 'g', time_axis, get_dev('mu'), 'r', 'LineWidth', 2);
title('Housing & Constraint (% Dev)'); legend('H_M', 'H_S', 'Multiplier (mu)', 'Location', 'best'); grid on; xlim([1 options_.periods]);

% Subplot 4: Total Consumption (% dev)
subplot(3,3,4);
plot(time_axis, get_dev('C'), 'k', 'LineWidth', 2);
title('Aggregate Consumption (% Dev)'); legend('Total Consumption (C)', 'Location', 'best'); grid on; xlim([1 options_.periods]);

% Subplot 5: Profits & Dividends (Levels - zero in SS)
subplot(3,3,5);
plot(time_axis, get_lvl('d_i'), 'b', time_axis, get_lvl('d_b'), 'r', time_axis, get_lvl('d_k'), 'g');
title('Profits (Levels)'); legend('Goods (d^i)', 'Banks (d^b)', 'Capital (d^k)', 'Location', 'best'); grid on; xlim([1 options_.periods]);

% Subplot 6: Transfers (Levels - some zero in SS)
subplot(3,3,6);
...
% Subplot 1: Period Utilities (Levels - can be negative)
subplot(2,2,1);
plot(time_axis, get_lvl('U_H'), 'b', time_axis, get_lvl('U_M'), 'r', time_axis, get_lvl('U_S'), 'g', 'LineWidth', 2);
title('Period Utility (Levels)'); legend('U_H', 'U_M', 'U_S', 'Location', 'best'); grid on; xlim([1 options_.periods]);

% Subplot 2: Welfare (Levels - can be negative)
subplot(2,2,2);
plot(time_axis, get_lvl('V_H'), 'b', time_axis, get_lvl('V_M'), 'r', time_axis, get_lvl('V_S'), 'g', 'LineWidth', 2);
title('Welfare (Levels)'); legend('V_H', 'V_M', 'V_S', 'Location', 'best'); grid on; xlim([1 options_.periods]);
...
% Subplot 1: Bank Deposit Funding
subplot(2,2,1);
plot(time_axis, get_dev('m'), 'b', 'LineWidth', 2);
title('Bank Deposit Funding (% Dev)'); legend('m', 'Location', 'best'); grid on; xlim([1 options_.periods]);

% Subplot 2: Bank Bonds & Mortgages
subplot(2,2,3);
plot(time_axis, get_dev('n'), 'r', time_axis, get_dev('b'), 'g', 'LineWidth', 2);
title('Bank Bonds & Mortgages (% Dev)'); legend('n','b', 'Location', 'best'); grid on; xlim([1 options_.periods]);

% Subplot 3: Bank Reserves & CB Loans
subplot(2,2,2);
